...
$$y_{ijk}= \mu_0 + \pi_i + T_{ij} \theta + c_j + e_{ijk}$$ 

with

* $y_{ijk}$ the response in cluster $j$ at time $i$ for individual $k$
* $\mu_0$ an overall mean (under control)
* $\pi_i$ the time trend at time $i$
* $T_{ij}$ indicates the treatment status (0 = control, 1 = interventional treatment)
* $\theta$ the treatment effect 
* $c_j$ a random cluster effect for cluster $j$ with $b_j \sim N(0,\tau^2)$
...
Let's define $\beta:=(\mu,\alpha',\theta)'$ and $\omega_{ijk}:=b_j+e_{ijk}$. 
...
$$y_{ijk}= \mu_0 + \pi_i + T (\theta_{ij} + d_j) + c_j + t_{ij} + s_{ijk} + e_{ijk}$$ 
with  

* $d_j$ a random treatment effect
* $c_j$ and $d_j$ jointly distributed with $\left(\begin{smallmatrix} \tau^2 & \rho\tau\eta \\ \rho\tau\eta & \eta^2\end{smallmatrix}\right)$
...
`compute_wlsPower` then calls `construct_DesMat` and `construct_CovMat`.
...
The `plot.wlsPower` method makes up to three plots, visualising the influence 
of cluster-periods, the treatment matrix $T$ and the covariance matrix 
(selectable by `which=`). 

By default, only the first plot is returned. It is constructed by exploiting the 
...
is also included in period $j+1$. 
...
There exist some formulae that do not need explicitly construct design and 
